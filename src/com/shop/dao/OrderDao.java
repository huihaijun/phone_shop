...
public interface OrderDao {
	...
	long orderReadCountByStatuss(int status,int statuss);
	//获得订单列表（分页）,条件用户id
	...
	List<Order> orderListByStatuss(PageBean pb, int status, int statuss);
	//查找订单编号通过订单号
	Order findOrderByOrderId(int orderId);
	//更改订单状态
	boolean orderStatus(int orderId,int state);

	boolean updateOrderReason(int orderId, String reason);
